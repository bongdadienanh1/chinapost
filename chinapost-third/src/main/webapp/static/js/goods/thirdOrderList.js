...
//添加卖家备注
function addNotes(orderId){
    $("#addNotes").attr("placeholder","填写卖家备注...");
    $("#addNotes").val("");
    $("#addNotesOrderId").val(orderId);
    $('#addNotesDialog').modal('show');
}

//保存卖家备注
function subAddNotes(){
    var orderId = $("#addNotesOrderId").val();
    var orderNotes = $("#addNotes").val();
    if(orderNotes == ''){
        alert("请填写卖家备注");
        return false;
    }
    $.post("updateThirdOrderNotes.htm", { orderId: orderId,orderNotes:orderNotes},
        function(data){ $('#addNotesDialog').modal('hide');});
}
...
function changePageNo(obj){
    $(".searchThirdOrderList").attr("action","queryThirdOrderList.html");
...
function queryOrderList(){
    $("#startTime").attr("name",'payTimeStart');
    $("#endTime").attr("name",'payTimeEnd');
    $(".searchThirdOrderList").attr("action","orderManage/getOrders");
    $('.searchThirdOrderList').submit();
}
//根据条件查询出库单信息
function queryThirdOutStock(){
    $(".searchThirdOrderList").attr("action","querythirdoutstock.html");
    $('.searchThirdOrderList').submit();
}
//更新订单
...
//导出第三方订单
function exportThirdOrder(){
    $(".searchThirdOrderList").attr('action','orderDown');
    searchOrder();
    $(".searchThirdOrderList").attr('action','getOrders');
}
