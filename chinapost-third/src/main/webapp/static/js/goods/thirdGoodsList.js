...
//所有商品
function outAllGoods(){
    $("#goodsPtoductId").val("");
    $("#outgoodsform").attr("action","exprotGoodsThird.htm");
    $("#outgoodsform").submit();
    $("#outgoodsform").attr("action","thirdgoodsmanager.htm");
}

//导出勾选商品
function outselect(){
    var goodsPtoductIds = new Array();
    var goodsIds = $("input[name=thirdGoodsId]");
    for (var i= 0; i<goodsIds.length;i++){
        var e = goodsIds[i];
        if (e.checked == true) {
            goodsPtoductIds.push(goodsIds.eq(i).val());
        }
    }
    if(goodsPtoductIds.length<=0){
        $(".modal-title").html("操作提示");
        $("#noout-tip").modal('show');
        return;
    }
    $("#goodsPtoductId").val(goodsPtoductIds);
    $("#outgoodsform").attr("action","exprotGoodsThird.htm");
    $("#outgoodsform").submit();
    $("#outgoodsform").attr("action","thirdgoodsmanager.htm");
}
...
//导出所有货品
function outAllItems(){
    $("#goodsPtoductId").val("");
    $("#outgoodsform").attr("action","exprotGoodsProduct.htm");
    $("#outgoodsform").submit();
    $("#outgoodsform").attr("action","thirdgoodsmanager.htm");
}
//导出选中货品
function outselectItems(){
    var goodsPtoductIds = new Array();
    var goodsIds = $("input[name=thirdGoodsId]");
    for (var i= 0; i<goodsIds.length;i++){
        var e = goodsIds[i];
