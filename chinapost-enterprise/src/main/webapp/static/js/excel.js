window.onload = function() {
    ...
    var seriesData = [{name:"已分配财富", value:wealth_do, color:"RGBA(84,166,222,1)"},
        {name:"未分配财富", value:wealth_undo, color:"RGBA(245,108,115,1)"}]
    $(".Ucoin #my_container abbr:first-child span").html(parseFloat(wealth_do/6250).toFixed(2));
    $(".Ucoin #my_container abbr:nth-child(2) span").html(parseFloat(wealth_undo/6250).toFixed(2))
    var config = {
        width : 400,
        height: 300,
        series: seriesData,
...
var pieChart = {
    ...
    render : function() {
        ...
        this.circle.radius = Math.min(this.width/2, this.height/2) - 1.2*Math.max(this.edge.width, this.edge.height);
        var ctx = null;
    ...
    renderPie : function(ctx, index, precent, deltaArc) {
        ...
        if(this.text.enable) {
            ...
            ctx.strokeStyle="black";
            ctx.stroke();
            var textPos = (hx < this.circle.cx) ? (hx - this.edge.width) : (hx);
            precent = Math.round (precent*100) / 100;
            var size = this.text.content.length;
            var tipStr = (size > index) ? this.text.content[index] : this.series[index].name + ": " + (precent * 100).toFixed(2) + "%";
            ctx.font = '10pt Calibri';
            ctx.fillStyle="black";
            if( this.series[index].name == '已分配财富' ){
    ...
    renderLegend : function(ctx, sum) {
        ...
        for(var i=0; i<nums; i++) {
            ...
            var tipStr =  this.series[i].name + ": " + this.series[i].value;
