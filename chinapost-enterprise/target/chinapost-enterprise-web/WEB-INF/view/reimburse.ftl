...
<body>
<input type="hidden" value="${goodsInfoType}" id="goodsInfoType">
    ...
    <dl>
        ...
        <#if  inventorybill.billStatus.name() == 'WAIT_EDIT'>
            <#list inventorybill.items as item>
                <#assign x=x+1>
                <dd data-id="${item.getInfo().getGoodsInfoId()}"><abbr><input type="checkbox" name="invetcheck"><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="topSee"><input value="${item.getAmount()}" class="change" type="text"> </abbr></dd></#list>
