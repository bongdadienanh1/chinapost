...
function loadProvice(pro) {
	...
	$.ajax({
		type : 'post',
		url : basePath+'/api/address/provincies',
		async : false,
		success : function(data) {
			var options = "";
			for (var i = 0; i < data.data.length; i++) {
				var province = data.data[i];
				if (province.provinceId == pro && pro!="") {
...
function loadCity(pid,cname) {
	...
	$.ajax({
				type : 'post',
				url :basePath+ '/api/address/cities',
				data : paramStr,
				async : false,
				success : function(data) {
					if (data.length != 0) {
...
function chooseA(aid){
	...
	$("#countyCh").val(aid);
	$(".choose_area .local").html("");
	$(".choose_area .local").html(choosedp+choosedc+chooseda);
	$('.area-box-lv3').hide();
	$('.opacity-bg-3').remove();
    $('.screen').hide();
}
