...
<div class="member_info">
    ...
    <dl>
        ...
        <dd>
            <#if phoneNo??&& phoneNo!=''>
                <a href="${basePath}/goPhoneNo" class="">
                    <span>${phoneNo!''}</span>
                </a>
            <#else >
                <a href="${basePath}/goPhoneNo" class="">
    ...
    <dl>
        <dt>联系电话</dt>
        <dd>
            <a href="javascript:;" class="contectphone">
...
<div class="gender_choose" style="display:none;">
    <a href="${basePath}/updateMaleAndPhone?male=true" class="btn btn-full-grey">
        <i class="iconfont icon-unie040"></i> 男
    </a>
    <a href="${basePath}/updateMaleAndPhone?male=false" class="btn btn-full-grey">
...
//    修改联系电话
    $('.contectphone').click(function(){
        var reasonBox = dialog({
            width : 260,
            title : '请输入您的联系电话',
            content : '<form id="upcontectphont"><input type="text" placeholder="请输您的联系电话..." style="width: 60%; margin-left: 20%;" id="contectphoneinput"><input type="hidden" value="" id="contecthidden" name="contactPhone"></form>',
            okValue : '确定',
            cancelValue : '取消',
            ok : function(){
//                $('.return_reason .curValue').text($('.reasons li.selected a').text());
//                $('#backReason').val($('.reasons li.selected a').attr('value'));
                var contectph = $('#contectphoneinput').val();
//                $.post('updateMaleAndPhone',{
//                            contactPhone:contectph
//                        },
//                        function(data){
//                            if(data.response == "success"){
//                                window.reload();
...
//                            }
//                        },'json');
                if(checkMobile(contectph)){
                    $('#contecthidden').val(contectph);
                    console.log( $('#contecthidden').val());
                    $("#upcontectphont").attr("action","${basePath}/updateMaleAndPhone");
                    $("#upcontectphont").submit();
                    return true;
                }
            },
            cancel : function(){
