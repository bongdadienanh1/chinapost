...
public class OrderController {
    ...
    public String submitOrder(HttpServletRequest request,Long addressId, Long[] shoppingCartId, boolean selfPick, String remark, Long enterpriseId, String paykey) {
        // 保存订单
        List<Order> orders = placeOrderService.submitOrder(addressId, shoppingCartId, selfPick, remark, enterpriseId,paykey);
        ...
        //付款
        placeOrderService.payOrder(orderCodes, paykey);
        if(selfPick){
            return new JsonResponseBean(placeOrderService.getDeliveryCode(orders.get(0).getOrderCode())).toJson();
        }else {
