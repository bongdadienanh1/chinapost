    ...
    <ul>
        ...
        <li style="float: right;margin-right: 1%;">
            <#if status == 'done' || status == 'myrequest'>
                <#if inventorybill.billStatus.name() == 'FINISHED' && !isEnd && !inventorybill.getReadFlag() &&isTop!=true>
                <div style="display: inline-block;" id="createAllocation"> <input type="button" class="topButton allclickButton" value="创建调拨单"> </div>
                </#if>
                <#if isRead != true && isTop && inventorybill.billStatus != 'TERMINATED'>
                    <div style="display: inline-block;" id="createPurchase"> <input type="button" class="topButton allclickButton" value="创建采购单"> </div>
                </#if>
            <#else>
                <#if inventorybill == ''>
                ...
                <#else>
                    ...
                    <#else>
                        <#if isTop == true && inventorybill.billStatus != 'CHECKED_NOT_PURCHASE' && inventorybill.billStatus != 'CHECKED_NOT_TRANSFER'>
                        ...
                        <#else>
                            <#if inventorybill.creatorId != enterpriseInfo.enterpriseId && inventorybill.billStatus != 'CHECKED_NOT_PURCHASE' && inventorybill.billStatus != 'CHECKED_NOT_TRANSFER'>
                                <div style="display: inline-block;" id="repleExamine"> <input type="button" class="topButton approve allclickButton" value="审核" class="approve"><input type="button" class="topButton nodoupt2 allclickButton" value="同意" class="nodoupt"><input type="button" class="topButton stop allclickButton" value="终止"></div>
                            </#if>
                        </#if>
                    </#if>
                    <#if inventorybill.billStatus == 'CHECKED_NOT_PURCHASE' && isTop>
                        <div style="display: inline-block;">
                            <input type="button" value="创建采购单" class="topButton creatOrderImmidently allclickButton">
                        </div>
                    </#if>
                    <#if inventorybill.billStatus == 'CHECKED_NOT_TRANSFER' && !isTop && !isEnd>
                        <div style="display: inline-block;">
                            <input type="button" value="创建调拨单" class="topButton creatOrderImmidently2 allclickButton">
                        </div>
                    </#if>
...
<div class="bodyFirst">
    ...
    <ul>
        ...
        <li style="width: 717px;"><span>*申请原因</span><abbr><input maxlength="20" readonly="readonly" style="border:none;" type="text" class="reasonForApply" value="${inventorybill.reason}"/> </abbr></li>
    </ul>
...
<div class="bodySecond">
    ...
    <dl>
        <dt><abbr>序号</abbr><abbr>货品名称</abbr><abbr>货品规格</abbr><abbr>货品编号</abbr><abbr>装箱数</abbr><abbr>可用库存</abbr><#if inventorybill == ''><abbr class="topSee">*申请数量</abbr><#else><#if isTop == true && inventorybill.billStatus.name() != 'CHECKING' ><#elseif isTop != true && inventorybill.billStatus.name() != 'CHECKING' ><abbr class="topSee">*申请数量</abbr><#elseif inventorybill.billStatus.name() == 'CHECKING' && isTop != true><abbr class="topSee">*申请数量</abbr></#if></#if><#if inventorybill == ''><#elseif inventorybill.billStatus.name() == 'CHECKED'><abbr class="topSee">*审核数量</abbr><#else><abbr class="topSee">*审核数量</abbr></#if>
        </dt>
        <#assign x = 0>
        <#if  inventorybill.billStatus.name() == 'CHECKING'>
            <#if isTop != true >
                <#list inventorybill.getItems() as item>
                    <#assign x=x+1>
                    <dd data-id="${item.getItemId()}"><abbr><input type="checkbox" name="invetcheck"><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="outNum">${item.getAmount()}</abbr><abbr class="topSee"><#if status == "done">${item.getCheckedAmount()}<#else><input type="text" class="change" value="${item.getAmount()}"></#if></abbr></dd>
                </#list>

            <#else>
                <#list inventorybill.getItems() as item>
                    <#assign x=x+1>
                    <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="topSee">${item.getCheckedAmount()}</abbr></dd>
                </#list>
            </#if>
        ...
        <#elseif  inventorybill == ''>
            <#list inventorybill.getItems() as item>
                <#assign x=x+1>
                <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr><input type="text"></abbr><abbr class="topSee"><input type="text" class="change"></abbr></dd>
            </#list>
        ...
        <#elseif inventorybill.billStatus.name() == 'FINISHED'>
            ...
            <#else>
                <#list inventorybill.getItems() as item>
                    <#assign x=x+1>
                    <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="outNum">${item.getAmount()}</abbr><abbr class="topSee">${item.getCheckedAmount()}</abbr></dd>
                </#list>
            </#if>
        <#elseif inventorybill.billStatus.name() == 'TERMINATED'>
            <#if isTop == true >
                <#list inventorybill.getItems() as item>
                    <#assign x=x+1>
                    <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="topSee">${item.getCheckedAmount()}</abbr></dd>
        ...
        <#elseif inventorybill.billStatus.name() == 'CHECKED_NOT_PURCHASE'>
                <#list inventorybill.getItems() as item>
                    <#assign x=x+1>
                    <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="topSee">${item.getCheckedAmount()}</abbr></dd>
                </#list>
        <#elseif inventorybill.billStatus.name() == 'CHECKED_NOT_TRANSFER'>
            <#list inventorybill.getItems() as item>
                <#assign x=x+1>
                <dd data-id="${item.getItemId()}"><abbr><b>${x}</b></abbr><abbr title="${item.getInfo().getGoodsInfoName()}">${item.getInfo().getGoodsInfoName()}</abbr><abbr>${item.getInfo().getSpecString()}</abbr><abbr>${item.getInfo().getGoodsInfoItemNo()}</abbr><abbr><b class="goodsInfoPack">${item.getGoodsInfoPack()}</b>${item.getGoodsInfoUnit()}/箱</abbr><abbr><#if item.getInfo().getAvailable()=="" >0<#else>${item.getInfo().getAvailable()}</#if></abbr><abbr class="topSee">${item.getCheckedAmount()}</abbr></dd>
            </#list>
