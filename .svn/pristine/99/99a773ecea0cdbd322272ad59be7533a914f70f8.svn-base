...
<div class="reTop">
    <ul>
        <li style="float: left;margin-left: 1%; color: #54a6de;font-weight: bold">
            <span style="margin-right: 10px;">补货申请流程</span>-<span class="state" style="margin-left: 10px;">
            审核中
        </span>
        </li>
        <li style="float: right;margin-right: 1%;">
            <#if isRead != true && status != 'myrequest'>
                <div style="display: inline-block;" id="repleAgree"><input maxlength="20" type="button" value="生成采购订单" class="topButton createPurchase allclickButton"></div>
            </#if>
        </li>
    </ul>
</div>
...
<div class="bodyFirst">
    ...
    <ul>
        <li><span>单据编号</span><abbr id="code">新合并补货单</abbr></li>
        <li><span>申请账号</span><abbr>${enterpriseInfo.enterpriseName}</abbr></li>
        <li>
            <span>申请日期</span>
            <abbr id="time"></abbr>
        </li>
        <li><span>当前状态</span><abbr class="state">审核中</abbr></li>
        <li style="width: 717px;"><span>备注</span><abbr id="reason"><input style="" type="text" class="reasonForApply" value=""/> </abbr></li>
    </ul>
...
<input id="list" value="${parentId}" type="hidden">
...
<script type="text/javascript">
    $(document).ready(function(){
        var type = 0
        var list = $("#list").val()
        $.post('../web/api/inventory/getMergeBillSumByBillId',{
            billId:list
        },function(data){
            if(data.response == "success"){
                if(data.data.billStatus == "TERMINATED"){
                    $("#repleAgree").css("display","none");
                }
                $(".state").html(handleInventoryStatus(data.data.billStatus))
                ...
                data.data.items.map(function(object){
                    ...
                    html += '<tr>'
                    html += '<td>'+ x +'</td>'
                    html += '<td>' + handleUndefined(object.info.goodsInfoName) + '</td>'
                    html += '<td>' + handleUndefined(object.info.specString) + '</td>'
                    html += '<td>' + handleUndefined(object.info.goodsInfoItemNo) + '</td>'
        ...
        $("#change").click(function(){
            if( type == 0){
                $.post('../web/api/inventory/getMergeBillByParentId',{
                    billId:list
                },function(data){
                    if(data.response == "success"){
                        if(data.data.billStatus == "TERMINATED"){
...
//                        $("#itemUnitShowHead").empty().append(html2)
        ...
        $(".stop").click(function(){
            ...
            if(reason != ""){
                ...
                response_ensure_alert("error","申请原因不能为空")
            }
        })
