...
public class GoodsSpecServiceImpl implements GoodsSpecService {
     ...
     * @return 查询到的规格列表
     */
    public List<GoodsSpec> queryAllSpec(String goodsInfoType){
        //查询所有的商品规格
        return this.goodsSpecMapper.queryAllSpec(goodsInfoType);
    }
        ...
        try {
            ...
            for (int i = 0; i < specDetails.size(); i++) {
                ...
                if (goodsSpecDetail.getSpecDetailId() != -1) {
                    ...
                    if (ValueUtil.ALREADYDELFLAG.equals(goodsSpecDetail.getSpecDetailDelflag())) {
                        // 调用规格值得Service方法进行删除
            ...
            //更新规格
            Map<String,Object> paramMap = MapUtil.getParamsMap(goodsSpec);
            return this.goodsSpecMapper.updateByPrimaryKeySelective(paramMap);
        } finally {
            // 参数置空
    ...
    @Override
    public int countGoodsBySpecId(String... specId) {
        return goodsSpecMapper.countGoodsBySpecId(specId);
    }
