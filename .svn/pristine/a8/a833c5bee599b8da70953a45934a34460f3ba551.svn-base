...
function updateRollAdv(rollAdvid){
	...
	for(var i=0;i<rolls.length;i++){
		...
		updateRollAdvEdit(src,href,i+1);
	}
...
function saveRollAdv(){
    ...
    var $count = $('.swiper-wrapper');
    var d = new Dictionary();
    // 验证排序值是否出现重复
    var arr = new Array([slides.length]);
	if(slides.length<1){
	...
	for(var i=0;i<slides.length;i++){
		var href = $(slides[i]).find(".custom-input");
        var sort = $(slides[i]).find(".sortBoss");
		if(href.val()!=""){
        ...
        if(sort.val()==""){
            toastr.error('第'+(i+1)+'张图片请输入序列号');
            flag = flag && false;
        }
        arr[i] = sort.val();
        // 添加轮播到js 键值对
        d.put('"d'+sort.val()+'"',$count.find(".swiper-slide:eq("+(i+1)+")").html());
	}
    ...
    // 重新排列轮播
    for(var i=0;i<slides.length;i++){
        $count.find(".swiper-slide:eq("+(i+1)+")").html(d.get('"d'+(i+1)+'"'));
    }
    ...
    if(flag){
		...
		$("#rollAdvEdit .slide").each(function(){
            ...
            if($(".app_selected .swiper-slide").eq(s).find("img").attr("src") == _img) {
                $(".app_selected .swiper-slide").eq(s).find("a").attr("href",src);
            };
...
function isRepeat(arr){
    ...
    for(var i=0;i<arr.length-1;i++) {
        if(nary[0] != 1){
            toastr.error('轮播图列表需要有1存在！~');
            return false;
        }
        if (nary[i] == nary[i + 1]) {
            toastr.error('轮播图排序不能重复！~');
            return false;
        }
        if(nary[i + 1] - nary[i] != 1){
            toastr.error('轮播图顺序不能跨跃！~');
            return false;
        }
    }
    return true;
...
function updateFullRoll(id){
	...
	clearFullRoll();
	//隐藏
	$("#advEdit, #cubeEdit,#cubeEditBy3, #rollAdvEdit, #textEdit, #basicEdit").hide();
	//显示
...
//		}     
	if(id=="adv"){//广告  
		...
		/*$(".app_selected .swiper-slide").each(function(){
            ...
            $(".app_selected .swiper-slide").eq(_index).find("img").attr("src",path);
		//});
		rollSwiper.reInit();
...
function saveChoooseProduct(id, img,no,name,title,price){
	for(var i=0;i<id.length;i++){
		var _li = '<li><a href="productdetail?productId='+id[i]+'"><img id="" width="100%" src="'+img[i]+'">'+
				  ...
				  '<span class="good-price">邮豆'+price[i]+'</span><em class="good-buy-btn">立即购买</em></div></a></li>';
		...
		+'<input type="hidden" name="name" value="'+name[i]+'">'
		+'<input type="hidden" name="price" value="邮豆'+price[i]+'">'
		+'</div><a class="rmv-gd" onclick="rmv(this)">X</a></div>';
		$(".gds-show").append(html);
...
function saveAllMod(){
	...
	console.log($("#ip_cont").html());
	$.ajax({
		url : "${bath}/saveAllMod.htm",
		type: 'POST',
		data : {
			ipCont : $("#ip_cont").html(),
		...
		async : false,
		success : function(){
			location = "queryMobHomePage.htm?flag=0";
		}
	});
}
...
KindEditor.ready(function(K) {
	editor = K.create('textarea[name="content"]', {
		cssPath : '${bath}/static/js/kindeditor/plugins/code/prettify.css',
		uploadJson : '${bath}/web/imgUpload',
		fileManagerJson : '${bath}/static/js/kindeditor/jsp/file_manager_json.jsp',
...
/*$(document).keydown(function (e) {
    switch(e.which) {
        case 37:
            var $cont = $('.slides_wp');
            var $count = $('.swiper-wrapper').children('.swiper-slide');
            $cont.append($cont.find('.slide:first'));
            $count.eq($count.length-2).after($count.eq(1));
            break;
        case 39:
            var $cont = $('.slides_wp');
            var $count = $('.swiper-wrapper').children('.swiper-slide');
            $cont.prepend($cont.find('.slide:last'));
            $count.eq(1).before($count.eq($count.length-2));
            break;
    }
});*/
