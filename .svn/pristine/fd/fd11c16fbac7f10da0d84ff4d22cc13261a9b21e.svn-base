...
//            productWare = null;
    ...
    @Override
    public void batchUpdateAuditStatus(HttpServletRequest request) {
        String auditStatus = request.getParameter("auditStatus");
        Map<String,Object> paramMap = new HashMap<String,Object>();
        if(!StringUtil.isBlank(auditStatus)){ // 批量操作
            paramMap.put("auditStatus",auditStatus);
            paramMap.put("ids",Arrays.asList(request.getParameterValues("goodsInfoIds")));
            if(auditStatus.equals("2")){
                paramMap.put("refuseReason",request.getParameter("refuseReason"));
            }
        }else{ // 单个操作
            String refuseReason = request.getParameter("refuseReason");
            if(!StringUtil.isBlank(refuseReason)){
                paramMap.put("refuseReason",refuseReason);
                paramMap.put("auditStatus","2");
            }else{
                paramMap.put("auditStatus","1");
            }
            paramMap.put("goodsInfoId",request.getParameter("goodsInfoId"));
        }
        goodsProductMapper.batchUpdateAuditStatus(paramMap);
    }
}
