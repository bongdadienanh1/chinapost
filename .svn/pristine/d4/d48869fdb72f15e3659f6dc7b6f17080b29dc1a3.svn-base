...
public class CustomerController {
    ...
    @ResponseBody
    public String checkExistsPayPassWord(HttpServletRequest request,String phoneNo) {
        long customerId = (long) request.getSession().getAttribute("customerId");
        request.getSession().setAttribute("_phoneNo",phoneNo);
    ...
    public String myaccount(HttpServletRequest request) {
        if (LoginUtil.checkLoginStatus(request)) {
            ...
            request.setAttribute("male", customer.getMale()+"");//性别  前台获取不到request中的boolean 中的false值 加"" 可以获取
            request.setAttribute("phoneNo", customer.getPhoneNo());
            request.setAttribute("idcardNo", customer.getIdcardNo());
            request.setAttribute("contactPhone", customer.getContactPhone());//联系电话
    ...
    public String updatePayPassword(HttpServletRequest request, String oldpayPassword, String payPassword, String newpassword, String flag) {
        ...
        if (flag.equals("2")){ //flag=2绑定的手机号码的 设置支付密码 跳转到的页面
            String phone = "";
            if(request.getSession().getAttribute("_phoneNo") != null){
                 phone=request.getSession().getAttribute("_phoneNo").toString();  //Object对象转String 不能为 NULL 否则报空指针
                 request.getSession().removeAttribute("_phoneNo");//释放资源
            }
            return new JsonResponseBean("goPhoneNo?phoneNo="+phone).toJson();
        }
        return JsonResponseBean.getSuccessResponse().toJson();
    }
    ...
    public String myUcoin(HttpServletRequest request) {
        if (LoginUtil.checkLoginStatus(request)) {
            ...
            request.setAttribute("customerUcoinHistoryList", customerUcoinHistoryService.getMobileHistory((Long) request.getSession().getAttribute("customerId"),new Pageable(1,15)));
            return "myUcoin";
        } else {
            // 没登录的用户跳转到登录页面
            return "redirect:/login";
    ...
    public String myUcoinPage(HttpServletRequest request,int pageNumber){
        if(LoginUtil.checkLoginStatus(request)){
            Map<String, Object> paramMap = new HashMap<String, Object>();
            paramMap.put("customerUcoinHistoryList", customerUcoinHistoryService.getMobileHistory((Long) request.getSession().getAttribute("customerId"), new Pageable(pageNumber, 15)));
    ...
    public String billDeta(HttpServletRequest request,Long customerId,Long batchId,HistoryType type,Long orderId) {
        if (LoginUtil.checkLoginStatus(request)) {
            request.setAttribute("customerUcoinHistory", customerUcoinHistoryService.selectDetailByCondition(customerId, batchId, type, orderId));
            return "billDetail";
        } else {
            // 没登录的用户跳转到登录页面
            return "redirect:/login";
     ...
     * @param phoneNo  手机号码  在编辑手机号码之前 验证密码
     * @return
     */
    @RequestMapping("/updateMaleAndPhone")
    public String updateMaleAndPhone(HttpServletRequest request,boolean male,String contactPhone,String phoneNo){
        if (LoginUtil.checkLoginStatus(request)) {
            ChinapostCustomer customer = currentLoginService.getCurrentLoginCustomer();
            chinapostCustomerService.updateMaleAndPhone(customer.getId(), male, contactPhone, phoneNo);
    ...
    /**
     * 跳转到phoneNumber页面修改手机号码
     */
    @RequestMapping("/goPhoneNo")
    public String goPhoneNo(HttpServletRequest request,String phoneNo){
        if (LoginUtil.checkLoginStatus(request)) {
            if(StringUtil.isBlank(phoneNo)){
                phoneNo = currentLoginService.getCurrentLoginCustomer().getPhoneNo();
            }
            request.setAttribute("customer", currentLoginService.getCurrentLoginCustomer());
    ...
    public String updateAddress(HttpServletRequest request,long provinceId,long cityId,long districtId,String addr){
        if (LoginUtil.checkLoginStatus(request)) {
           ...
           /* return "redirect:/contactAddress";*/
                 return "redirect:/account";//跳转到个人中心页面
        } else {
            // 没登录的用户跳转到登录页面
            return "redirect:/login";
        }
