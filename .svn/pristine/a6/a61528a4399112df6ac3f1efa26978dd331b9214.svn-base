...
import com.ylife.chinapost.mobile.util.IndexStaticizeUtil;
import com.ylife.main.model.*;
import com.ylife.main.service.*;
...
public class MobMainSiteController {
     ...
     * 视图路径
     */
    private static final String TEMPLATE_PATH = new ClasspathPropertiesHelper("config.properties").getProperty("REAL_PATH");
    ...
    @Resource
    private MobPageTagService mobPageTagService;
    ...
    public ModelAndView initMain(HttpServletRequest request) throws Exception {
        ...
        if ("1".equals(msb.getTemp2())) {
            // 拿取活动对象
            mav.addObject("mobPageTag",mobPageTagService.getPageTagAllBySort(1));
            //SEO
            mav.addObject("seo", getSeo());
            //站点设置
            mav.addObject("sys", basicSetService.findBasicSet());
...
//            mav.setViewName("main/store" + STOREID + "/new_main");
    ...
    public ModelAndView subject(HttpServletRequest request,Long subjectId,Long merchantId) throws Exception {
        ModelAndView mav = new ModelAndView();
        MobSiteBasic msb = mobSiteBasicService.selectCurrMobSiteBasic(request);
        mav.addObject("seo", getSeo());
        mav.addObject("sys", basicSetService.findBasicSet());
        ...
        // 判断专题页面路径是否存在，不存在创建一个
        checkFileExistsAndMkdirsSubject(request , subjectId);
        // 生成专题文件
        try {
            showNewSubject(request, subjectId);
        }catch (NullPointerException e){
            return new ModelAndView(new RedirectView("initMain.htm"));
        }
        mav.setViewName("main/store" + STOREID + "/new_subject");
        return mav;
    }
    ...
    /**
     * 显示专题 获取数据库的xml，写入服务器的用户xml文件，并根据xsl生成html
     */
    private void showNewSubject(HttpServletRequest request , Long subjectId) throws Exception {
        MobSubject homePage = this.mobSubjectService.getCurrSubjectByMerchantId(subjectId);
        Document document = xmlUtil.str2Document(homePage.getDoc());
        String filePath = request.getSession().getServletContext().getRealPath("/");
        String htmlFilePathSubject = "view/main/Subject_"+ subjectId +".html";
        String htmlFileName = filePath + htmlFilePathSubject;
        htmlFileName = htmlFileName.replace("/main", "/main/store" + STOREID);
        // 判断是否有html
        if (new File(htmlFileName).exists()) {
            SimpleDateFormat format =  new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            // 文件存在时
            Date ndate = format.parse(format.format(new File(htmlFileName).lastModified())); // 原文件的修改时间
            Date odate = homePage.getUpdateDate();
            if(odate.compareTo(ndate)>0){
                // 文件不存在，生成html
                String xmlFileName = xmlFilePath.replace(".xml", "_" + STOREID + ".xml");
                String fileName = filePath + xmlFileName;
                // 把doc写入用户的文件
    ...
    * 创建专题模板
    */
    private void checkFileExistsAndMkdirsSubject(HttpServletRequest request,Long subjectId) throws Exception {
        // 获取用户首页文件夹路径
        String rootPath = request.getSession().getServletContext().getRealPath("/");
        File storeFile = new File(rootPath + "view/main/store" + STOREID);
        ...
        try {
            ...
            oldPath = rootPath + "view/main/new_subject.ftl";
            newPath = rootPath + "view/main/store" + STOREID + "/new_subject.ftl";
            oldfile = new File(rootPath + "view/main/new_subject.ftl");
            newfile = new File(rootPath + "view/main/store" + STOREID + "/new_subject.ftl");
            if (oldfile.exists()) { // 文件存在时
                if(newfile.exists()){ // 需建文件存在时
                    SimpleDateFormat format =  new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    Date odate = format.parse(format.format(oldfile.lastModified())); // 原文件的修改时间
                    Date ndate = format.parse(format.format(newfile.lastModified())); // 新文件的修改时间
                    // 源文件的修改时间大于新文件的修改时间的时候  更新新文件
                    if(odate.compareTo(ndate)>0){
                        try (InputStream inStream = new FileInputStream(oldPath); FileOutputStream fs = new FileOutputStream(newPath)) {
                            ...
                            LOGGER.debug("new_*.ftl文件大小=" + bytesum);
                        } catch (final Exception e) {
                            LOGGER.error("复制new_*.ftl操作错误", e);
                            throw e;
                        }
                    }
                }else{
                ...
                LOGGER.error("没找到new_*.ftl");
                throw new RuntimeException("没找到new_*.ftl");
            }
        } catch (final Exception e) {
            LOGGER.error("创建用户文件夹失败", e);
            throw e;
        }
    }
