...
<div class="order content-order-detail mb50">
    ...
    <div class="all-info mt10">
        <div class="list-item">
            <ul class="price-total">
                ...
                <li>
                    <label for="">订单总价（含运费）</label>
                    <span class="value text-them" id="sPrice">
                        <#if order.moneyPaid??> ${order.moneyPaid!''}
                        <#else>0
                        </#if>
                        邮豆
                    </span>
   ...
   <#if order.isValet?string('true','false')=='true'>
    ...
    <div class="bar-bottom" style="height: 92px;">
                        ...
                        <#if order.orderStatus=="SUBMITED">
                            <#--<#if order.otherAmount??&& order.ucoinPrise??>-->
                                <#--<#if (order.otherAmount!'0.00')?string('0.00')!='0.00'&&(order.ucoinPrise)?string('0.00')!='0.00'>-->
                                    ...
                                    <#--</#if>-->
                                    <#--<#else>-->
                                        <p class="allPrice" style="margin-bottom: 20px"><label for="">待支付:</label><span style="color: red" ><#if order.moneyPaid??> ${(order.moneyPaid)?string('0.00')}<#else>0.00</#if>邮豆</span></p>
                                        <a href="javascript:void(0);" class="btn" onClick="cancelOrder('${order.orderNo}')">取消订单</a>
                                        <#if order.payId==1>
                                            <a href="javascript:void(0);" class="btn pay-btn" onclick="payOrder('${order.orderNo}','${order.moneyPaid}')">立即支付</a>
                                        </#if>
                                <#--</#if>-->
                            <#--</#if>-->
                        <#elseif order.orderStatus=="PAYED">
