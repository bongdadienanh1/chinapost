...
<div class="member_form">
  ...
  <input type="hidden" value="${orderaddress.addressId!''}" name="addressId" id="addressId">
  ...
  <dl>
    ...
    <dd>
      ...
      <a href="javascript:;" class="choose_area" onclick="loadProvice('${orderaddress.province.provinceName!''}');">
        <span class="local" style="padding: 5px;">${orderaddress.province.provinceName!''}${orderaddress.city.cityName!''}${orderaddress.district.districtName!''}
  ...
  <dl>
    <dt>详细地址</dt>
    <dd>
      <textarea rows="4" class="text" name="addressDetail" id="addressDetail" style="padding: 5px;">${orderaddress.addressDetail!''}</textarea>
    </dd>
  </dl>
  <dl class="check_item">
    <dt>设为默认</dt>
  ...
  //验证表单
    ...
    $(function(){
        if($("#addressId").val() == ""|| $("#addressId").val() == null){
            $("#addressName").attr("placeholder","收货人");
            $("#addressMoblie").attr("placeholder","手机号码");
            $(".choose_area").find("span").text("请选择");
            $("#addressDetail").attr("placeholder","详细地址");
            $("#addForm").attr("action","${basePath}/orderaddaddress");
        }
    })
</script>
</body>
