...
function querySpecDetail(specId) {
    $("#updateSpecDetailForm #specId").val(specId);
    /*ajax 通过ID查询并塞值到修改页面*/
    $.get("queryById?specId=" + specId, function (data) {
        /*设置规格值*/
        var html = "";
        for (var i = 0; i < data.specDetails.length; i++) {
            html+=
                ...
                '   <td><input type="text" class="form-control required specstr" name="specDetailName" maxlength="5" value="'+ data.specDetails[i].specDetailName + '"></td>'+
                '   <td><input type="text" class="form-control required number" name="specDetailSort" onKeyUp="onKeyupfun(this)"value="' + data.specDetails[i].specDetailSort + '"></td>'+
               /*'   <td><button type="button" class="btn btn-default" onclick="deleteOneSpecDetail(\'_'+data.specDetails[i].specDetailId+'\')">删除</button></td>'+*/
               '<td><button type="button" class="btn btn-default" onclick="checkGoodsBySpecDetailId(\'_'+data.specDetails[i].specDetailId+'\','+data.specDetails[i].specDetailId+')">删除</button></td>'+
                '</tr>';
        }
...
function addOneSpecDetail() {
   ...
   var  html =
        ...
        '   <td><input type="text" class="form-control required specstr" name="specDetailName" maxlength="5"></td>'+
        '   <td><input type="text" class="form-control number" name="specDetailSort" onKeyUp="onKeyupfun(this)" value="0"></td>'+
 ...
 * @param specDetailId
 */
function checkGoodsBySpecDetailId(index,specDetailId){
    if(specDetailId){
        $.ajax({
            url:'countGoodsBySpecDetailId',
            data:{specDetailIds:specDetailId},
            success:function (data) {
                if(data == 200){
                    deleteOneSpecDetail(index);
                }else{
                    showTipAlert(data);
                }
            }
...
function deleteCheckGoodsBySpecDetailId(name){
    var specDetailIds = [];
    /*for(var j = 0; j < checkboxs.length; j++) {
        specDetailIds.push($(checkboxs[j]).val().toString());
    }*/

    $("#spec_detail_table tbody input[name="+name+"]").each(function(){
        specDetailIds.push($(this).val());
    });

    $.ajax({
        url:'countGoodsBySpecDetailId',
        data:{specDetailIds:specDetailIds.join(",")},
        success:function (data) {
            if(data == 200){
                batchDeleteSpecDetail();
            }else{
                showTipAlert(data);
            }
        }
