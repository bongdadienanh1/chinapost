$(function () {
    ...
    $('#btnLoadDetail').click(function(){
        var httpUrl = $(this).parents('.add_good_form').find(".httpUrl").val();
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data != "" && data != null) {
                    ...
                    $(this).parents('.add_good_item').next().find(".品牌").find(".type_param_val").val("ceshi");

                    for (var i = 0; i < data.outPrams.length; i++) {
                        try {
                            $(this).parents('.add_good_item').find(".paramTables").find("." + data.outPrams[i].outParam).find(".type_param_val").val(data.outPrams[i].outValue);
                        } catch (e) {
                            continue;
                        }
                    }

                    //$(this).parents(".common_info_cont").find(".name_input").val();
                    $('input[name="goodsNameTemp"]').val(data.goodName);
    ...
    if(null != pro_box && pro_box.length>0){
        for(var j=0;j<pro_box.length;j++){
            ...
            if(_pro.find(".product_spec").length>0){
                ...
                param+="&goodsInfoUnit="+$(".proUnit").val();
                param+="&goodsInfoPack="+$(".proPack").val();
                ...
                /*获取是否显示在列表页或商城*/
                if (_pro.find(".show_list").prop("checked") && _pro.find(".show_mobile").prop("checked")) {
                    param += "&showList=1&showMobile=1";
                }else if (_pro.find(".show_mobile").prop("checked")){
                    param += "&showList=1&showMobile=1";
                }else if(_pro.find(".show_list").prop("checked")){
                    param += "&showList=1";
                    param += "&showMobile=0";
                }
